...
namespace Aryan\soundcloud;

use Exception;
use stdClass;

class SoundCloud extends HttpRequest
{
    ...
    public function searchMusic(string $query): stdClass|array
    {
        try {
            $musics = [];
            $searchData = $this->endpoint("{$this->soundCloudApiUrl}search/tracks", [
                'q' => $query,
    ...
    {
        try {
            $musicData = $this->endpoint("{$this->soundCloudApiUrl}tracks", [
                'ids' => $id,
                'client_id' => $this->getClientId(),
    ...
    public function downloadMusic(string $id): void
    {
        ...
        $directly = false;
        if (isset($dataMusic->media->transcodings[1])) {
            $url = $dataMusic->media->transcodings[1]->url;
            $directly = true;
        } else {
